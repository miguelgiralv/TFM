...
mashr_n_genes_processed=count_genes_in_files_processed("results/predictXcan/test/vcf_1000G_hg37_mashr/processed", "processed_mashr_", "mashr_usados")
...
mashr_n_genes.head()
mashr_n_genes_processed.head()
# como el procesado tiene los mismos genes que el no procesado, predixcan no predice expresion de los genes de los cuales no tiene SNP
# para sacar el numero de genes del modelo, necesitamos procesar el numero de filas de los summary:

def count_genes_in_files2(custom_path, file_prefix, model_name):
...
mashr_n_genes=count_genes_in_files2("results/predictXcan/test/vcf_1000G_hg37_mashr", "mashr_", "mashr")
en_n_genes=count_genes_in_files2("results/predictXcan/test/vcf_1000G_hg37_en", "en_", "en")
...
type(n_genes_tissues.columns)


lista=n_genes_tissues.columns.values.tolist()
...
def count_genes_in_files_processed(custom_path, file_prefix, model_name):
    ...
    genes_df_final = pd.DataFrame(genes_df.items(), columns=['Tejido', model_name])
    return genes_df_final, gene_list

en_n_genes_processed=count_genes_in_files_processed("results/predictXcan/test/vcf_1000G_hg37_en/processed", "processed_en_", "proc_en")

gene_list=en_n_genes_processed[1]
unique_gene_list = list(set(gene_list))

len(unique_gene_list) # numero de genes unicos en total usados en todos los tejidos
