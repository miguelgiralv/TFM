...
load(paste0(path, "results/netactivity/se_list3_curated.RData"))
...
all_pvalues[order(all_pvalues$P.value), ]
...
all_pvalues_FDR<-read.table(paste0(path,"results/netactivity/","FDR_curated.txt"))
...
######################PARA METASCORE, SELECCION DE GOS "BUENOS"
filtered_gos <- all_pvalues_2[all_pvalues_2$FDR < 0.25 |
    all_pvalues_2$Bonferroni < 0.25 &
    (all_pvalues_2$logFC > 0.05 | all_pvalues_2$logFC < -0.05), ]
...
# almacenaremos los gos y su tejido en orden en dos listas separadas en orden
go_list<-list()
tissue_list<-list()
...
data_list <- list()
# almacenamos en un df
for (i in 1:nrow(filtered_gos_subset)) 
{
GO_name<-filtered_gos$GO[i]
go_tissue_name<-filtered_gos$tissue[i]
   
# extraemos el summarisedexperiment del tejido de interes
score_tissue <- score_tissue_list[[go_tissue_name]]
#transponemos su df
assay_t <- as.data.frame(t(assay(score_tissue)))
assay_t <- rownames_to_column(assay_t, "Sample")
unique_go_name<-paste0(GO_name,"_",go_tissue_name)
data_list[[unique_go_name]] <- assay_t[[GO_name]]
print("hecho")
}
combined <- as.data.frame(data_list)

# ahora le metemos los metadatos con el ultimo valor de score_tissue
coldata <- as.data.frame(colData(score_tissue))
rownames(coldata) <- NULL
# lo unimos con combined
...
# nos faltaria seleccionar el 80% de las muestras de forma random
set.seed(123)
n_rows <- nrow(merged_data)
n_sample <- round(0.8 * n_rows)
sample_indices <- sample(1:n_rows, n_sample)
df_sampled <- merged_data[sample_indices, ]
df_unsampled<-merged_data[-sample_indices, ]

# pasamos a numerico y estandarizamos la expresion

exclude_cols <- c("Sample", "DiseaseStatus", "Sex")
for (col_name in colnames(df_sampled)) {
  if (!(col_name %in% exclude_cols)) {
    # Convert column to numeric if it is not already
    df_sampled[[col_name]] <- as.numeric(as.character(df_sampled[[col_name]]))
    # Scale the numeric column
    df_sampled[[col_name]] <- scale(df_sampled[[col_name]])
  }
}
df_sampled<-df_sampled %>% select(-"Sample")



save(score_tissue_list, file = paste0(path, "results/netactivity/se_list5_curated.RData"))


# hacemos el modelo
model <- glm(DiseaseStatus ~ ., data = df_sampled, family = binomial)
...
##¿seleccion de solo los de p valor <0.05?
coef_summary <- coef_summary[coef_summary[["Pr(>|z|)"]] < 0.10 | coef_summary[["Predictors"]]=="(Intercept)", ]

score_list<-list()
# metascore
for (col in colnames(df_unsampled))
{
  if (col %in% coef_summary$Predictors)
  {
  estimate_value <- coef_summary[coef_summary[["Predictors"]] == col, "Estimate"]
  values<-df_unsampled[[col]]*estimate_value
  }  
score_list[[col]]<-values
print("hecho")
}
combined_scores <- as.data.frame(score_list)
intercept_value <- coef_summary[coef_summary[["Predictors"]] == "(Intercept)", "Estimate"]
combined_scores$intercept=-intercept_value
final_score <- data.frame(Metascore = numeric(nrow(combined_scores)))
final_score$Metascore <- rowSums(combined_scores)

final_score$DiseaseStatus<-df_unsampled$DiseaseStatus

final_score$DiseaseStatus <- as.factor(final_score$DiseaseStatus) 
final_score$Metascore <- as.numeric(final_score$Metascore)  
final_score$Metascore<-scale(final_score$Metascore)

model_metascore <- glm(DiseaseStatus ~ ., data = final_score, family = binomial) 
summary(model_metascore)
exp(coef(model_metascore))
